  ...
  return (
    <main className='min-h-screen bg-gray-50 pb-24'>
      <div className='bg-red-500 px-6 pt-10 pb-6'>
        <h1 className='text-2xl font-bold text-white'>Catalogo BioTokens</h1>
        <p className='text-red-100 text-sm'>Canjea tus tokens por premios</p>
      </div>
      <div className='px-4 py-6 grid grid-cols-2 gap-4'>
        {cargando ? (
          ...
          <div className='col-span-2 bg-white rounded-2xl p-8 shadow-sm text-center'>
            <p className='text-gray-500 text-sm'>No hay premios disponibles</p>
          </div>
        ) : (
          productos.map(p => (
            <Link href={'/biotokens/' + p.id} key={p.id}>
              <div className='bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col h-56'>
                ...
                <div className='p-3 flex flex-col flex-1 justify-between'>
                  ...
                  <p className='text-red-500 font-bold text-sm mt-1'>{p.precio_tokens} tokens</p>
                </div>
              </div>
            </Link>
          ))
