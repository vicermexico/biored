...
export default function Biotokens() {
  const [productos, setProductos] = useState<any[]>([])
  const [cargando, setCargando] = useState(true)
  useEffect(() => {
    fetch('/api/productos/biotokens').then(r => r.json()).then(data => { setProductos(data); setCargando(false) }).catch(() => setCargando(false))
  }, [])
  return (
    <main className='min-h-screen bg-gray-50 pb-24'>
      <div className='bg-red-500 px-6 pt-10 pb-6'><h1 className='text-2xl font-bold text-white'>Catalogo BioTokens</h1><p className='text-red-100 text-sm'>Canjea tus tokens por premios</p></div>
      <div className='px-6 py-6 grid grid-cols-2 gap-4'>
        {cargando ? (<><div className='bg-gray-200 rounded-2xl h-48 animate-pulse'></div><div className='bg-gray-200 rounded-2xl h-48 animate-pulse'></div></>) : productos.length === 0 ? (<div className='col-span-2 bg-white rounded-2xl p-8 shadow-sm text-center'><p className='text-4xl mb-3'>🎁</p><p className='text-gray-500 text-sm'>No hay premios disponibles</p></div>) : (productos.map(p => (<Link href={'/biotokens/' + p.id} key={p.id}><div className='bg-white rounded-2xl overflow-hidden shadow-sm'><div className='bg-gray-100 h-36 flex items-center justify-center'>{p.foto_url ? <img src={p.foto_url} alt={p.nombre} className='w-full h-full object-cover' /> : <span className='text-4xl'>🎁</span>}</div><div className='p-3'><p className='font-semibold text-sm text-gray-800'>{p.nombre}</p><p className='text-xs text-gray-400 mt-1'>{p.descripcion_corta}</p><p className='text-red-500 font-bold mt-2'>{p.precio_tokens} tokens</p></div></div></Link>)))}
      </div>
