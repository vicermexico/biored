      ...
      <div className='px-6 py-6 flex flex-col gap-4'>
        <div className='bg-white rounded-2xl p-6 shadow-sm flex flex-col items-center gap-3'>
          <div className='w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center'>
            <svg xmlns='http://www.w3.org/2000/svg' className='w-10 h-10 text-gray-900' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' /></svg>
          </div>
          <p className='font-medium text-gray-800'>{usuario.nombre}</p>
        </div>
        <div className='bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-4'>
          {guardado && <p className='text-gray-700 text-sm text-center font-medium bg-gray-100 py-2 rounded-xl'>Guardado correctamente</p>}
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input type='text' value={nombre} onChange={e => setNombre(e.target.value)} className='border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-500' />
          </div>
          <div className='flex flex-col gap-1'>
            <label className='text-sm text-gray-600 font-medium'>Celular</label>
            <input type='tel' value={celular} onChange={e => { const val = e.target.value.replace(/\D/g, ''); if (val.length <= 10) setCelular(val) }} maxLength={10} className='border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-500' />
          </div>
          <div className='flex flex-col gap-1'>
            <label className='text-sm text-gray-600 font-medium'>Correo</label>
            <input type='email' value={correo} onChange={e => setCorreo(e.target.value)} className='border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-500' />
          </div>
          <div className='flex flex-col gap-1'>
            <label className='text-sm text-gray-600 font-medium'>Nuevo NIP</label>
            <input type='password' value={nip} onChange={e => setNip(e.target.value)} placeholder='Deja vacio para no cambiar' maxLength={4} className='border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-500' />
          </div>
          <Button onClick={handleGuardar} disabled={cargando} className='w-full bg-gray-900 hover:bg-black text-white font-semibold py-6 rounded-2xl'>
