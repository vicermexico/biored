    ...
    <main className='min-h-screen bg-gray-50 pb-24'>
      ...
      <div className='px-6 py-6 flex flex-col gap-4'>
        <div className='flex gap-2 overflow-x-auto pb-1'>
          {[1,2,3,4,5].map(n => (
            <button key={n} onClick={() => setNivelActivo(n)} className={'px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ' + (nivelActivo === n ? 'bg-gray-900 text-white' : 'bg-white text-gray-500 border border-gray-200')}>
              Nivel {n}
            </button>
          ))}
        </div>
