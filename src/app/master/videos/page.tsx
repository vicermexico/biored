      ...
      <div className="px-6 py-6 flex flex-col gap-4">
        {(modo === 'agregar' || modo === 'editar') && (
          <div className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-3 border-2 border-green-500">
            ...
            <input type="text" placeholder="Titulo (opcional)" value={form.titulo} onChange={e => setForm({ ...form, titulo: e.target.value })} className="border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none" />
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Cuantas veces se mostrara al usuario?</label>
              <input type="number" min={1} value={form.veces_mostrar} onChange={e => setForm({ ...form, veces_mostrar: parseInt(e.target.value) || 1 })} className="border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none" />
            </div>
            {modo === 'agregar' && (
              <div className="flex flex-col gap-1">
        ...
        {cargando ? (
          ...
          videos.map(v => (
            <div key={v.id} className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-3">
              <div>
                <p className="font-medium text-gray-800">{v.titulo || 'Sin titulo'}</p>
                <p className="text-xs text-gray-400 mt-1">Se muestra {v.veces_mostrar} {v.veces_mostrar === 1 ? 'vez' : 'veces'}</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => toggleActivo(v.id, v.activo)} className={`flex-1 py-2 rounded-xl text-sm font-medium ${v.activo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                  {v.activo ? 'Activo' : 'Inactivo'}
                </button>
                <button onClick={() => handleEditar(v)} className="bg-blue-50 text-blue-500 px-4 py-2 rounded-xl text-sm font-medium">
                  Editar
                </button>
                <button onClick={() => eliminar(v.id)} className="bg-red-50 text-red-500 px-4 py-2 rounded-xl text-sm font-medium">
                  Eliminar
                </button>
              </div>
            </div>
