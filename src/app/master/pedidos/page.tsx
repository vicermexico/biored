        ...
        <div className="flex gap-2">
          {['hoy', 'semana', 'mes', 'todos'].map(f => (
            <button
              key={f}
              onClick={() => setFiltro(f)}
              className={`px-3 py-1.5 rounded-xl text-sm font-medium capitalize ${filtro === f ? 'bg-green-700 text-white' : 'bg-white text-gray-600'}`}
            >
              {f}
            </button>
          ))}
          ...
          pedidosFiltrados.map(p => (
            <div key={p.id} className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-2">
              ...
              <div className="flex justify-between text-sm font-bold border-t border-gray-100 pt-2">
                <span>Total</span>
                <span>{p.tipo === 'biotokens' ? `${p.total_tokens} tokens` : `$${p.total?.toFixed(2)}`}</span>
              </div>
