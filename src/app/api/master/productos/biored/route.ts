...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from('productos_biored').insert({ nombre, descripcion_corta, descripcion_larga, precio, foto_url, video_url, fotos_adicionales, activo: true }).select().single()
...
export async function PATCH(request: Request) {
  ...
  const { error } = await supabase.from('productos_biored').update({ nombre, descripcion_corta, descripcion_larga, precio, foto_url, video_url, fotos_adicionales }).eq('id', id)
